...
- **Hard gates** sit after The deal and before contact. Pass: lives in the US (`country` `US`), uses an iPhone (`device` `iPhone`), and takes `1–2`, `3–5`, or `6+` different supplements on a typical day (`supplement_count`, not frequency). Fail — No (`Outside US`), Android, Other, or `0` — opens the existing later-round page and does not POST `/api/apply`. **Under 18** is still a hard stop on the age question. "I don't fit this round" on the deal screen still opens that same later-round page. Passing answers are written as Notion Device = iPhone and Country = US.
